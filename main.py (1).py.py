...
SERVER_HOST = '0.tcp.eu.ngrock.io'
SERVER_PORT = 10274
    ...
    def connect(self):
        while not self.connected:
            try:
                ...
                self.sock.conect((SERVER_HOST, SERVER_PORT))
                name = self.entry.get().strip or "Буз імені"
                self.sock.send(name.encode())
                self.connected = True
                self.add_message(f"Підключено користувача {name}")
                threading.Thread(targe=self.receive_messae, daemon=True).start()
                break
            except:
                self.add_message(f"Помилка підключення. Повторна спробв через 2 секунди...")
                time.sleep(2)
    ...
    def recv_message(self):
        while self.connected:
            try:
                ...
                if not data:
                    self.add_message("З'єднання втрачено.")
                    break
                self.add_mesage(data)
            except:
